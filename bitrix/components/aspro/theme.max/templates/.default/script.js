...
$(document).ready(function () {
  ...
  getWidgetHtml = function (callback) {
    ...
    if ($(".top_block_switch").hasClass("loading")) return;
  ...
  $(document).on("click", ".style-switcher .switch", function (e) {
    ...
    var topBlockSwitch = _this.closest("top_block_switch");
    if (checkNeedRequest()) {
      _this.addClass("loadings");
      // }

      //   if (topBlockSwitch.hasClass("loading")) {
      timerResponse = setInterval(function () {
        if (!checkNeedRequest() || !hoveredSwitcher) {
