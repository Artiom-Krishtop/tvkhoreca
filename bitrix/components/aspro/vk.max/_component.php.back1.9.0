...
if(\Bitrix\Main\Loader::includeModule('aspro.max')){
	...
	$arResult['TOKEN'] = CMax::GetFrontParametrValue('API_TOKEN_VK');
	$arResult['GROUP_ID'] = CMax::GetFrontParametrValue('GROUP_ID_VK');
	if (intval($arResult['GROUP_ID']) > 0) {
		$arResult['GROUP_ID'] = '-'.$arResult['GROUP_ID'];
	}
	$arResult['TITLE'] = CMax::GetFrontParametrValue('VK_TITLE_BLOCK');
	$arResult['ALL_TITLE'] = CMax::GetFrontParametrValue('VK_TITLE_ALL_BLOCK');
	$arResult['TEXT_LENGTH'] = CMax::GetFrontParametrValue('VK_TEXT_LENGTH');
