...
//if(is_string($arParams["ELEMENT_SORT_FIELD"])){
if(strlen((string)$arParams["ELEMENT_SORT_FIELD"])<=0)
...
if($this->StartResultCache(false, array($arrFilter, ($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $arNavigation)))
{
	...
	//if(is_string($arParams["ELEMENT_SORT_FIELD"])){
	$arSort = array(
		(string)$arParams["ELEMENT_SORT_FIELD"] => $arParams["ELEMENT_SORT_ORDER"],
		"ID" => "DESC",
	);
    echo "ss";
//	}
	...
	//xmp(array_merge($arrFilter, $arFilter));
	...
	/*if(isset($_REQUEST["scroll2item"])){
		unset($arNavParams["bShowAll"]);
		$arNavParams["nElementID"] = $_REQUEST["scroll2item"];
