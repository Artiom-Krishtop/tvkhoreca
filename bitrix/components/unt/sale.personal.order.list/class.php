...
class CBitrixPersonalOrderListComponent extends CBitrixComponent
{
	 ...
	 * @return void
	 */
	protected function prepareFilter($userId = false)
	{
		global $USER;
		global $DB;

		$arFilter = array();
		$arFilter["USER_ID"] = $this->arParams["USER_ID"];
	 ...
	 * @return int Order id
	 */
	private function getRealId($id, $userId)
	{
		global $USER;
		...
		if ($this->options['USE_ACCOUNT_NUMBER'])
		{
			$dbOrder = $saleOrder->GetList(
				array("ID"=>"DESC"),
				array("ACCOUNT_NUMBER" => $id, "USER_ID" => $this->arParams["USER_ID"], "LID" => SITE_ID),
				false,
				false,
				array("ID")
			);
		...
		if (!$arOrder)
		{
			$dbOrder = $saleOrder->GetList(
				array("ID"=>"DESC"),
				array("ID" => $id, "USER_ID"=> $this->arParams["USER_ID"], "LID" => SITE_ID),
				false,
				false,
				array("ID")
			);
