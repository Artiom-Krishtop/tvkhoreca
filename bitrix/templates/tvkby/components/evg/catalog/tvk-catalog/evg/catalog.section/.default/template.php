			...
			<table cellpadding="15" cellspacing="5" border="0" >
				<tr>
					...
					<?if(strlen($arElement["PROPERTIES"]["image"]["VALUE"]) > 0):?>
						<td class="item-photo">
						<a name="<?=$arElement["ID"]?>" href="<?=$arElement["DETAIL_PAGE_URL"]?>"><img border="0" src="<?=$arElement["PROPERTIES"]["image"]["VALUE"]?>" width="135" height="" alt="<?=$arResult["NAME"]?>" title="<?=$arResult["NAME"]?>" /></a><br />
						</td>
					<?else:?>
						<td class="item-photo">
