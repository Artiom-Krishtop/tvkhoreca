...
/* Цены Товаров/Разделов товаров/Брендов товаров, которые необходимо скрыть от не авторизованного пользователя */
if(!$USER->IsAuthorized() && function_exists('isHidePrice')){
	foreach ($arResult['ITEMS'] as $key => $arItem) {
		if(isHidePrice($arItem)){
			$arResult['ITEMS'][$key]['CAN_BUY'] = false;

			if(!empty($arResult['ITEMS'][$key]['PRICES'])){
				foreach ($arResult['ITEMS'][$key]['PRICES'] as $priceID => $arPrice) {
					$arResult['ITEMS'][$key]['PRICES'][$priceID]['CAN_ACCESS'] = '';
					$arResult['ITEMS'][$key]['PRICES'][$priceID]['CAN_BUY'] = 'N';
				}
			}
		}
	}
