...
<form id = "catalog-filter-form" action="<?=$arParams["SECTION_URL"]?>search/">
	...
	<select name="material_id" onchange="$('#catalog-filter-form').submit()">
		...
		<?foreach ($arResult["FILTER_MATERIAL"] as $arMaterial):?>
			<option <?if($arResult["current_material_id"] == $arMaterial["PROPERTY_MATERIAL_VALUE"] && strlen($arMaterial["PROPERTY_MATERIAL_VALUE"])>0):?>selected<?endif;?> value="<?=$arMaterial["PROPERTY_MATERIAL_VALUE"]?>"><?=$arMaterial["PROPERTY_MATERIAL_VALUE"]?></option>
		<?endforeach;?>
	</select>			
</form>
</div>
...
<?endif;?>
<table cellpadding="0" cellspacing="0" border="0" style="width:825px!important;">
		...
		<td valign="top" width="185px" id="<?=$this->GetEditAreaId($arElement['ID']);?>">

			<table cellpadding="0" cellspacing="5" border="0" width="185">
				<tr>
					<?if(strlen($arElement["PROPERTIES"]["image"]["VALUE"]) > 0):?>
						<td class="item-photo" >
						<a name="<?=$arElement["ID"]?>" href="<?=$arElement["DETAIL_PAGE_URL"]?>"><img border="0" src="<?=$arElement["PROPERTIES"]["image"]["VALUE"]?>" alt="<?=$arElement["NAME"]?>" title="<?=$arElement["NAME"]?>" /></a><br />
						</td>
					<?else:?>
						<td class="item-photo">
						<a name="<?=$arElement["ID"]?>" href="<?=$arElement["DETAIL_PAGE_URL"]?>"><img border="0" src="/bitrix/templates/tvk-retail/images/no-photo.jpg" width="135px" height="105px" alt="<?=$arElement["NAME"]?>" title="<?=$arElement["NAME"]?>" /></a><br />
						</td>
					<?endif?>
</tr><tr>
