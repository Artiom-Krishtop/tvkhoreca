	...
	<?}?>
		...
		<?foreach($arResult["ITEMS"] as $arItem){?>
			<div class="catalog_item_wrapp">
				...
				$arQuantityData = CMShop::GetQuantityArray($totalCount, $arItemIDs["ALL_ITEM_IDS"], false, $arParams["MIN_AMOUNT"], $arParams["MAX_AMOUNT"]);
					...
					<div>
						...
						<div class="item_info <?=$arParams["TYPE_SKU"]?>">
							...
							<?foreach($arItem["PROPERTIES"] as $propCode => $arProp):?>
								<?if(isset($arItem["DISPLAY_PROPERTIES"][$propCode])):?>
									<?$arProp = $arItem["DISPLAY_PROPERTIES"][$propCode];?>
									<?if(!in_array($arProp["CODE"], array("SERVICES", "BRAND", "HIT", "RECOMMEND", "NEW", "STOCK", "VIDEO", "VIDEO_YOUTUBE", "SERIYA"))):?>
										<?if((!is_array($arProp["DISPLAY_VALUE"]) && strlen($arProp["DISPLAY_VALUE"])) || (is_array($arProp["DISPLAY_VALUE"]) && implode('', $arProp["DISPLAY_VALUE"]))):?>
											<div class="item-stock"><b><?=$arProp["NAME"]?>: </b>
													<?if(count($arProp["DISPLAY_VALUE"]) > 1):?>
														<?=implode(', ', $arProp["DISPLAY_VALUE"]);?>
													<?else:?>
														<?if($propCode=="TORGOVAYA_MARKA"):?>
															<a href="<?=$arItem["BRAND_ITEM"]["DETAIL_PAGE_URL"]?>"><?=$arProp["DISPLAY_VALUE"];?></a>
														<?else:?>
								...
								<?endif;?>
							<?endforeach;?>
							<?if($arItem["SERIA_LINK"]):?>
							<div class="item-stock"><b><?=$arItem["SERIA_LINK"]["NAME"]?>: </b><a href="<?=$arItem["SERIA_LINK"]["LINK"]?>"><?=$arItem["SERIA_LINK"]["VALUE"]?></a></div>
							<?endif;?>
							<?if($arItem["STATUS"] && $arParams["SHOW_STATUS"]!="N"):?>
								<div class="item-stock"><b><?=$arItem["STATUS"]["NAME"]?>: </b><?=$arItem["STATUS"]["VALUE"]?></div>
							<?endif;?>
<?if($arParams["SHOW_STATUS"]!="N"){?>
							<?=$arQuantityData["HTML"];?>
<?}?>
                            <?
                            $priceValue = false;
                            if($arItem["PRICES"])
                            {
                                foreach ($arItem["PRICES"] as $item)
                                    if (isset($item["VALUE"]) && $item["VALUE"] > 0)
                                        $priceValue = true;
                            }
                            ?>
                            <?if($arItem["OFFERS"] || $priceValue):?>
...
<?//if(2==1){?>
								...
								<?}?>
								<?if(!$arItem["OFFERS"] || $arParams['TYPE_SKU'] !== 'TYPE_1'):?>
									<?if(($arAddToBasketData["ACTION"] == "ORDER"/*&& !$arItem["CAN_BUY"]*/)  || $arItem["CAN_BUY"] || !$arAddToBasketData["OPTIONS"]["USE_PRODUCT_QUANTITY_LIST"] || $arAddToBasketData["ACTION"] == "SUBSCRIBE"):?>
