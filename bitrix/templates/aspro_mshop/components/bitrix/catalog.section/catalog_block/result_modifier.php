...
if (!empty($arResult['ITEMS'])){
	...
	foreach ($arResult['ITEMS'] as $key => $arItem)
	{
		...
		if($arItem["PROPERTIES"]["STATUS"]["VALUE"]){
			$arItem["STATUS"] = array("NAME"=> $arItem["PROPERTIES"]["STATUS"]["NAME"], "VALUE"=> $arItem["PROPERTIES"]["STATUS"]["VALUE"]);
		}
		if($arItem["PROPERTIES"]["SERIYA"]["VALUE"]){
			$arItem["SERIA_LINK"] = array("NAME"=> $arItem["PROPERTIES"]["SERIYA"]["NAME"], "VALUE"=> $arItem["PROPERTIES"]["SERIYA"]["VALUE"], "LINK"=> $arResult[LIST_PAGE_URL]."?q=".$arItem["PROPERTIES"]["SERIYA"]["VALUE"]."&how=r");
		}
...
if(strlen($arItem["DISPLAY_PROPERTIES"]["BRAND"]["VALUE"]) && $arItem["PROPERTIES"]["BRAND"]["LINK_IBLOCK_ID"]){
	$arResult['ITEMS'][$key]['BRAND_ITEM'] = CMshopCache::CIBLockElement_GetList(array('CACHE' => array("MULTI" =>"N", "TAG" => CMshopCache::GetIBlockCacheTag($arItem["PROPERTIES"]["BRAND"]["LINK_IBLOCK_ID"]))), array("IBLOCK_ID" => $arItem["PROPERTIES"]["BRAND"]["LINK_IBLOCK_ID"], "ACTIVE"=>"Y", "ID" => $arItem["DISPLAY_PROPERTIES"]["BRAND"]["VALUE"]));

}
}
?>
<?
if($arResult["NAV_RESULT"]->PAGEN > $arResult["NAV_RESULT"]->NavPageCount || $arResult["NAV_RESULT"]->PAGEN==1){
	$arResult["CANONICAL"] = $arResult["SECTION_PAGE_URL"];
}else{
	$arResult["CANONICAL"] = $arResult["SECTION_PAGE_URL"]."?PAGEN_".$arResult["NAV_RESULT"]->NavNum."=".(int)$arResult["NAV_RESULT"]->NavPageNomer;
	$arResult["REAL_PAGE_NUMBER"] = (int)$arResult["NAV_RESULT"]->NavPageNomer;
}
$cp = $this->__component; 
if (is_object($cp))
$cp->SetResultCacheKeys(array('CANONICAL','REAL_PAGE_NUMBER','NAV_RESULT','SECTION_PAGE_URL'));
?>