...
<div class="bx_ordercart basket_wrapp">
					...
					?>
						<td class="itemphoto thumb-cell">
							...
							<?if( strlen($arItem["PREVIEW_PICTURE_SRC"])>0 ){?>
								<?if (strlen($arItem["DETAIL_PAGE_URL"]) > 0):?><a href="<?=$arItem["DETAIL_PAGE_URL"]?>" class="thumb" target="_blank"><?endif;?>
					...
					foreach ($arResult["GRID"]["HEADERS"] as $id => $arColumn):
						...
						?>
							<td class="item name-cell <?=$class_td;?>" style1="width:<?=$width?>%">
								...
								<?if (strlen($arItem["DETAIL_PAGE_URL"]) > 0):?><a href="<?=$arItem["DETAIL_PAGE_URL"] ?>" target="_blank"><?endif;?>
									<?=$arItem["NAME"]?>
								<?if (strlen($arItem["DETAIL_PAGE_URL"]) > 0):?></a><?endif;?>
						...
						<?elseif ($arColumn["id"] == "PRICE_FORMATED"):?>
							<td class="custom cost-cell <?=( $bPriceType ? 'notes' : '' );?> <?=$class_td;?>">
								<?//print_r($arItem)?>
	...
	<div class="bx_ordercart_order_pay">
		...
		<div style="clear:both;"></div>
		<div class="bx_section_bottom" style="display:none;">
