				...
				<?if($arResult['ITEMS_BY_SECTIONS'][$SID]):?>
					<?if(count($arResult['SECTIONS']) > 1):?>
						...
						<div class="staff_section" id="<?=$this->GetEditAreaId($SID)?>">
							<div class="staff_section_title"><h4><!--<a rel="nofollow" href="">--><?=$SName?><!--</a>--><span class="slide opener_icon no_bg" style="display:none;"></span></h4></div>
					<?endif;?>
							<div class="staff_section_items" style="display: block;">
								<?foreach($arResult['ITEMS_BY_SECTIONS'][$SID] as $arItem):?>
									...
									<div class="item <?=($bShowImage ? '' : 'wi')?>" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
										<?if($bShowImage):?>
											<div class="image">
												<?if(!empty($arItem["PREVIEW_PICTURE"]) && in_array('PREVIEW_PICTURE', $arParams['FIELD_CODE'])):?>
													<?$arImage = CFile::ResizeImageGet($arItem["PREVIEW_PICTURE"]["ID"], array('width' => 300, 'height' => 200), BX_RESIZE_IMAGE_EXACT);?>
										...
										<div class="info">
											<?if($bShowName):?>
												<div class="name">
													<?=$firstName?><?if(strlen($secondName)):?>&nbsp;<?=$secondName?><?endif;?>
												</div>
											<?endif;?>
											...
											<?if(strlen($arItem["DISPLAY_PROPERTIES"]["PHONE"]["VALUE"])):?>
												<div class="phone"><?=GetMessage('PHONE')?>:&nbsp;<a rel="nofollow" href="tel:<?=$arItem["DISPLAY_PROPERTIES"]["PHONE"]["VALUE"]?>"><?=$arItem["DISPLAY_PROPERTIES"]["PHONE"]["VALUE"]?></a></div>
											<?endif;?>
											<?if(strlen($arItem["DISPLAY_PROPERTIES"]["EMAIL"]["VALUE"])):?>
												<div class="email"><?=GetMessage('EMAIL')?>:&nbsp;<a rel="nofollow" href="mailto:<?=$arItem["DISPLAY_PROPERTIES"]["EMAIL"]["VALUE"]?>"><?=$arItem["DISPLAY_PROPERTIES"]["EMAIL"]["VALUE"]?></a></div>
											<?endif;?>
										</div>						
									</div>
								<?endforeach;?>
