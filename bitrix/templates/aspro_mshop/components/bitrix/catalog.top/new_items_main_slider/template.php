...
?>
<?if($arResult["ITEMS"]):?>
	<div class="specials_slider_wrapp <?=$class_block;?>">
		<ul class="tabs" style=" padding-right: 0px;">
			<?foreach($arResult["ITEMS"] as $specials_code => $arItems):?>
				<li data-code="<?=$specials_code?>"><span><?=GetMessage($specials_code."_TITLE");?></span></li>
			<?endforeach;?>
			<li class="stretch"></li>
		</ul>
<a href="/catalog/novinki/">Все новые товары</a>
								...
								<div class="item_info">
									...
									<div class="cost prices clearfix">
										...
										<?elseif($arItem["PRICES"]):?>
											...
											<?foreach($arItem["PRICES"] as $key => $arPrice):?>
												<?if($arPrice["CAN_ACCESS"] && $arPrice["VALUE"]):
													$percent=0;?>
                                    ...
                                    ?>
									<div class="buttons_block clearfix"><?=($arAddToBasketData["HTML"] ? $arAddToBasketData["HTML"] : '')?></div>
								</div>
							</li>
						<?endforeach;?>
					</ul>
    ...
    <script>
        $(document).ready(function(){
            $('.specials_slider_wrapp.<?=$class_block;?> .tabs > li').first().addClass('cur');
            $('.specials_slider_wrapp.<?=$class_block;?> .slider_navigation > li').first().addClass('cur');
            $('.specials_slider_wrapp.<?=$class_block;?> .tabs_content > li').first().addClass('cur');

            var flexsliderItemWidth = 210;
            var flexsliderItemMargin = 20;
            var sliderWidth = $('.specials_slider_wrapp.<?=$class_block;?>').outerWidth();
            var flexsliderMinItems = Math.floor(sliderWidth / (flexsliderItemWidth + flexsliderItemMargin));
            $('.specials_slider_wrapp.<?=$class_block;?> .tabs_content > li.cur').flexslider({
                animation: 'slide',
            ...
            $('.specials_slider_wrapp.<?=$class_block;?> .tabs > li').click(function(){
                if(!$(this).hasClass('active')){
                    var sliderIndex = $(this).index();
                    $(this).addClass('active').addClass('cur').siblings().removeClass('active').removeClass('cur');
                    $('.specials_slider_wrapp.<?=$class_block;?> .slider_navigation > li:eq(' + sliderIndex + ')').addClass('cur').show().siblings().removeClass('cur');
                    $('.specials_slider_wrapp.<?=$class_block;?> .tabs_content > li:eq(' + sliderIndex + ')').addClass('cur').siblings().removeClass('cur');
                    if(!$('.specials_slider_wrapp.<?=$class_block;?> .tabs_content > li.cur .flex-viewport').length){
                        $('.specials_slider_wrapp.<?=$class_block;?> .tabs_content > li.cur').flexslider({
                            animation: 'slide',
                            selector: '.specials_slider > li',
                            slideshow: false,
            ...
            $(window).resize(function(){
                ...
                $('.specials_slider_wrapp.<?=$class_block;?> .tabs_content .tab.cur .specials_slider .buttons_block').hide();
                $('.specials_slider_wrapp.<?=$class_block;?> .tabs_content > li.cur').equalize({children: '.item-title'});
                $('.specials_slider_wrapp.<?=$class_block;?> .tabs_content > li.cur').equalize({children: '.item_info'});
                $('.specials_slider_wrapp.<?=$class_block;?> .tabs_content > li.cur').equalize({children: '.catalog_item'});

                var itemsButtonsHeight = $('.specials_slider_wrapp.<?=$class_block;?> .tabs_content .tab.cur .specials_slider > li .buttons_block').height();
                var tabsContentUnhover = $('.specials_slider_wrapp.<?=$class_block;?> .tabs_content .tab.cur').height() * 1;
                var tabsContentHover = tabsContentUnhover + itemsButtonsHeight+50;
                $('.specials_slider_wrapp.<?=$class_block;?> .tabs_content .tab.cur').attr('data-unhover', tabsContentUnhover);
                $('.specials_slider_wrapp.<?=$class_block;?> .tabs_content .tab.cur').attr('data-hover', tabsContentHover);
                $('.specials_slider_wrapp .tabs_content').height(tabsContentUnhover);
                if($('.specials_slider_wrapp.<?=$class_block;?> .tabs_content > li.cur').length && typeof($('.specials_slider_wrapp.<?=$class_block;?> .tabs_content > li.cur').data('flexslider')) !== 'undefined'){
                    $('.specials_slider_wrapp.<?=$class_block;?> .tabs_content > li.cur').data('flexslider').vars.minItems = flexsliderMinItems = Math.floor(sliderWidth / (flexsliderItemWidth + flexsliderItemMargin));
                    //	$('.specials_slider_wrapp .tabs_content > li.cur').data('flexslider').vars.maxItems = 3;
                    $('.specials_slider_wrapp.<?=$class_block;?> .tabs_content > li.cur').flexslider(0);
                }
                if($('.specials_slider_wrapp.<?=$class_block;?> .tabs_content > li.cur').find('.specials_slider > li').length > flexsliderMinItems){
                    $('.specials_slider_wrapp.<?=$class_block;?> .slider_navigation > li.cur').show();
                    $('.specials_slider_wrapp.<?=$class_block;?> .slider_navigation > li.cur > ul').show();
                }
                else{
                    $('.specials_slider_wrapp.<?=$class_block;?> .slider_navigation > li.cur').hide();
                    $('.specials_slider_wrapp.<?=$class_block;?> .slider_navigation > li.cur > ul').hide();
                }
            });
            $(window).resize();
            $('.<?=$class_block;?> .specials_slider > li').hover(
                function(){
