...
if($arResult["PROPERTIES"]["SERIYA"]["VALUE"]){
	$db_resSeria = CIBlockElement::GetList(Array("SORT"=>"ASC"), Array("IBLOCK_ID"=>$arParams['IBLOCK_ID'], "PROPERTY_451_VALUE"=>$arResult["PROPERTIES"]["SERIYA"]["VALUE"], "!ID"=>$arResult["ID"]), false, Array ("nTopCount" => 5), array('ID'));
	while ($resSeria = $db_resSeria->GetNext()) {
			$arResult["SERIA_ITEMS"][] = $resSeria["ID"];
	}
}
if(count($arResult["SERIA_ITEMS"])<5){
	$db_resSeria = CIBlockElement::GetList(Array("SORT"=>"ASC"), Array("IBLOCK_ID"=>$arParams['IBLOCK_ID'], "SECTION_ID"=>$arResult["IBLOCK_SECTION_ID"], "!ID"=>$arResult["ID"]), false, Array ("nTopCount" => (5-count($arResult["SERIA_ITEMS"]))), array('ID'));
	while ($resSeria = $db_resSeria->GetNext()) {
			$arResult["SERIA_ITEMS"][] = $resSeria["ID"];
	}
}
...
if($arResult["PROPERTIES"]["SERIYA"]["VALUE"]){
$arResult["SERIA_LINK"] = array("NAME"=> $arResult["PROPERTIES"]["SERIYA"]["NAME"], "VALUE"=> $arResult["PROPERTIES"]["SERIYA"]["VALUE"], "LINK"=> $arResult[LIST_PAGE_URL]."?q=".$arResult["PROPERTIES"]["SERIYA"]["VALUE"]."&how=r");
}
?>