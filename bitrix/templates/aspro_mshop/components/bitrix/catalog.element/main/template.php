...
$arQuantityData = CMShop::GetQuantityArray($totalCount, $arItemIDs["ALL_ITEM_IDS"], "Y", $arParams["MIN_AMOUNT"], $arParams["MAX_AMOUNT"]);
...
<div class="item_main_info <?=(!$showCustomOffer ? "noffer" : "");?>" id="<?=$arItemIDs["strMainID"];?>" itemscope itemtype="http://schema.org/Product">
							...
							<li id="photo-<?=$i?>" <?=(!$i ? 'class="current"' : 'style="display: none;"')?>>
								<?if(!$isEmpty){?>
									<a href="<?=$arImage["BIG"]["src"]?>" <?=($bIsOneImage ? '' : 'rel="item_slider"')?> class="fancy" title="<?=$title;?>">
										<img border="0" src="<?=$arImage["SMALL"]["src"]?>" alt="<?=$alt;?>" title="<?=$title;?>" itemprop="image" />
									</a>
								<?}else{?>
									<img border="0" src="<?=$arImage["SRC"]?>" alt="<?=$alt;?>" title="<?=$title;?>" itemprop="image" />
								<?}?>
							</li>
						<?}
					}
		...
		<div class="info_item">
					...
					<div class="wrap_md">
						<?if($arResult["BRAND_ITEM"]){?>
							<div class="brand iblock">
								<?
								$hideLink = false;
								$checkBrandUrl = 'https://www.tvkhoreca.by'.$arResult["BRAND_ITEM"]["DETAIL_PAGE_URL"];
								$headers = get_headers($checkBrandUrl);
								...
								<?if(!$arResult["BRAND_ITEM"]["IMAGE"]):?>
									...
									<?if($hideLink){?>
										<a href="<?=$arResult["BRAND_ITEM"]["DETAIL_PAGE_URL"]?>" title="<?=$arResult["BRAND_ITEM"]["NAME"]?>"><?=$arResult["BRAND_ITEM"]["NAME"]?></a>
									<?}else{?>
										<span><?=$arResult["BRAND_ITEM"]["NAME"]?></span>
									<?}?>
								<?else:?>
									<?if($hideLink){?>
						...
						<?if(((!$arResult["OFFERS"] && $arParams["DISPLAY_WISH_BUTTONS"] != "N") || ($arParams["DISPLAY_COMPARE"] == "Y")) || (strlen($arResult["DISPLAY_PROPERTIES"]["CML2_ARTICLE"]["VALUE"]) || ($arResult['SHOW_OFFERS_PROPS'] && $showCustomOffer))):?>
							<div>
								...
								<?endif;?>
									<h1 class="prod_inner_title" itemprop="name"><?=$arResult["IPROPERTY_VALUES"]["ELEMENT_PAGE_TITLE"];?></h1>
								...
								<?if(strlen($arResult["DISPLAY_PROPERTIES"]["CML2_ARTICLE"]["VALUE"]) || ($arResult['SHOW_OFFERS_PROPS'] && $showCustomOffer)):?>
									<div class="article iblock" <?if($arResult['SHOW_OFFERS_PROPS']){?>id="<? echo $arItemIDs["ALL_ITEM_IDS"]['DISPLAY_PROP_ARTICLE_DIV'] ?>" style="display: none;"<?}?> style="display: none;">
			...
			<?}else{?>
				<div class="top_info">
					<div class="wrap_md">
						<div>
					 		<h1 style="margin:0px" itemprop="name"><?=$arResult["IPROPERTY_VALUES"]["ELEMENT_PAGE_TITLE"];?></h1>
					 		<meta itemprop="description" content="<?=$arResult["IPROPERTY_VALUES"]["ELEMENT_META_DESCRIPTION"];?>">
						</div>
					</div>
				</div>
			<?}?>
			<div class="middle_info wrap_md" itemprop="offers" itemscope itemtype="http://schema.org/Offer">
								...
								if($arParams["SHOW_OLD_PRICE"]=="Y"){?>
									<div class="price" id="<? echo $arItemIDs["ALL_ITEM_IDS"]['PRICE']; ?>">
										<?if(strlen($minPrice["PRINT_DISCOUNT_VALUE"])):?>
											цена без НДС: <?=$prefix;?> <?=$minPrice["PRINT_DISCOUNT_VALUE"];?>
				...
				<div class="buy_block iblock">
					...
					<?if(!$arResult["OFFERS"]):?>
						...
						<div class="counter_wrapp">
							...
							<?endif;?>
							<?if((($arAddToBasketData["ACTION"] == "ORDER" /*&& !$arResult["CAN_BUY"]*/) || $arResult["CAN_BUY"] || !$arAddToBasketData["OPTIONS"]["USE_PRODUCT_QUANTITY_DETAIL"] || ($arAddToBasketData["ACTION"] == "SUBSCRIBE" && $arResult["CATALOG_SUBSCRIBE"] == "Y")) && $arResult["ACTIVE"]=="Y" ):?>
					...
					<?elseif($arAddToBasketData["ACTION"] == "SUBSCRIBE" && $arResult["CATALOG_SUBSCRIBE"] == "Y"):?>
							<div id="<? echo $arItemIDs["ALL_ITEM_IDS"]['BASKET_ACTIONS']; ?>" class="button_block <?=(($arAddToBasketData["ACTION"] == "ORDER" /*&& !$arResult["CAN_BUY"]*/) || !$arResult["CAN_BUY"] || !$arAddToBasketData["OPTIONS"]["USE_PRODUCT_QUANTITY_DETAIL"] || ($arAddToBasketData["ACTION"] == "SUBSCRIBE" && $arResult["CATALOG_SUBSCRIBE"] == "Y")  ? "wide" : "");?>">
								<!--noindex-->
									<?=$arAddToBasketData["HTML"]?>
								<!--/noindex-->
							...
							<?if(isset($arResult["DISPLAY_PROPERTIES"][$propCode])):?>
								...
								<?if(!in_array($arProp["CODE"], array("SERVICES", "BRAND", "HIT", "RECOMMEND", "NEW", "STOCK", "VIDEO", "VIDEO_YOUTUBE",))):?>
									<?if((!is_array($arProp["DISPLAY_VALUE"]) && strlen($arProp["DISPLAY_VALUE"])) || (is_array($arProp["DISPLAY_VALUE"]) && implode('', $arProp["DISPLAY_VALUE"]))):?>
										<div class="char">
											<div class="char_name">
												<b <?if($arProp["HINT"] && $arParams["SHOW_HINTS"] == "Y"){?>class="whint"<?}?>><?if($arProp["HINT"] && $arParams["SHOW_HINTS"] == "Y"):?><div class="hint"><span class="icon"><i>?</i></span><div class="tooltip"><?=$arProp["HINT"]?></div></div><?endif;?><?=$arProp["NAME"]?></b>
											</div>
											<div class="char_value">
												<?if(count($arProp["DISPLAY_VALUE"]) > 1):?>
													<?=implode(', ', $arProp["DISPLAY_VALUE"]);?>
												<?else:?>
<?if($propCode=="TORGOVAYA_MARKA"):?>
<a href="<?=$arResult["BRAND_ITEM"]["DETAIL_PAGE_URL"]?>"><?=$arProp["DISPLAY_VALUE"];?></a>
<?else:?>
...
<?endif;?>
							...
							<?endif;?>
						<?endforeach;?>
						<?if($arResult["SERIA_LINK"]):?>
							<div class="char">
								<div class="char_name">
									<b><?=$arResult["SERIA_LINK"]["NAME"]?></b>
								</div>
								<div class="char_value">
								 	<a href="<?=$arResult["SERIA_LINK"]["LINK"]?>"><?=$arResult["SERIA_LINK"]["VALUE"]?></a>
								</div>
							</div>
						<?endif;?>
						<?if($arResult["ACTIVE"]=="Y"){?>
...
<?if(strlen($arResult["DETAIL_TEXT"]) /*|| strlen($arResult["PROPERTIES"]["ITEM_DESCRIPTION"]["VALUE"]["TEXT"])*/):?>
<div class="tabs_section half">
	<ul class="tabs1 main_tabs1 tabs-head">
		...
		<?endif;?>
		<?if($arResult["DETAIL_TEXT"] /*|| $arResult["PROPERTIES"]["ITEM_DESCRIPTION"]["VALUE"]["TEXT"]*/ || count($arResult["STOCK"]) || count($arResult["SERVICES"]) || ((count($arResult["PROPERTIES"]["INSTRUCTIONS"]["VALUE"]) && is_array($arResult["PROPERTIES"]["INSTRUCTIONS"]["VALUE"])) || count($arResult["SECTION_FULL"]["UF_FILES"])) ):?>
	...
	<ul class="tabs_content tabs-body">
											...
											<!--noindex-->
												...
												<div class="wrap_md">
													...
													<div class="buy iblock ablock">
														<div class="counter_wrapp">
															<?//=$arskuAddToBasketData["HTML"] смотри сюда?>
														</div>
													</div>
												</div>
		...
		<?endif;?>
		<?if($arResult["DETAIL_TEXT"] /*|| $arResult["PROPERTIES"]["ITEM_DESCRIPTION"]["VALUE"]["TEXT"]*/ || count($arResult["STOCK"]) || count($arResult["SERVICES"]) || ((count($arResult["PROPERTIES"]["INSTRUCTIONS"]["VALUE"]) && is_array($arResult["PROPERTIES"]["INSTRUCTIONS"]["VALUE"])) || count($arResult["SECTION_FULL"]["UF_FILES"])) || ($showProps && $arParams["PROPERTIES_DISPLAY_LOCATION"] != "TAB")):?>
			<li class="<?=(!($iTab++) ? ' current' : '')?>">
				<?if(strlen($arResult["PROPERTIES"]["ITEM_DESCRIPTION"]["VALUE"]["TEXT"]) && 2==1):?>
					<div class="detail_text"><?echo htmlspecialcharsBack($arResult["PROPERTIES"]["ITEM_DESCRIPTION"]["~VALUE"]);?></div>
				<?endif;?>
...
<?endif?>
<?if(count($arResult["SERIA_ITEMS"])>0):?>
<?if(strlen($arResult["DETAIL_TEXT"]) /*|| strlen($arResult["PROPERTIES"]["ITEM_DESCRIPTION"]["VALUE"]["TEXT"])*/):?>
<div class="semi_propducts">
<?endif?>
	<?$GLOBALS['arrFilterSeria'] = array('ID' => $arResult["SERIA_ITEMS"]);?>
	<?$APPLICATION->IncludeComponent(
		"bitrix:catalog.top",
		"products_slider_viewed",
		array(
			"TITLE_BLOCK" => "Персональные рекомендации",
			"IBLOCK_TYPE" => $arParams["IBLOCK_TYPE"],
			"IBLOCK_ID" => $arParams["IBLOCK_ID"],
			"FILTER_NAME" => 'arrFilterSeria',
