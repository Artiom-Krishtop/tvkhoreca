...
if($section){
	...
	if(strlen($section["UF_MIN_AMOUNT"])){
		$arSection["UF_MIN_AMOUNT"] = $section["UF_MIN_AMOUNT"];
		$arParams["MIN_AMOUNT"] = $arSection["UF_MIN_AMOUNT"];
	}
	if(strlen($section["UF_MAX_AMOUNT"])){
		$arSection["UF_MAX_AMOUNT"] = $section["UF_MAX_AMOUNT"];
		$arParams["MAX_AMOUNT"] = $arSection["UF_MAX_AMOUNT"];
	}
	if(strlen($section["DESCRIPTION"]))
		$arSection["DESCRIPTION"] = $section["~DESCRIPTION"];
	if(strlen($section["DEPTH_LEVEL"]))
	...
	if(strlen($section["UF_SECTION_SEO_DESCR"]))
		$arSection["UF_SECTION_SEO_DESCR"] = $section["UF_SECTION_SEO_DESCR"];
...
?>
<?if($iSectionsCount > 0 && $arSection["DEPTH_LEVEL"]<2):?>
...
<?
	...
	$time = CIBlockSection::GetList(array("timestamp_x"=>"desc"),$arFilter, false, array("TIMESTAMP_X"), array("nTopCount"=>"1"))->Fetch();
	$time = abs(crc32($time["TIMESTAMP_X"]));

	$cache = new CPHPCache();
	$cache_time = 36000000;
	$cache_id = 'arIBlockSeoCatalogMenu_'.$time.$arResult["VARIABLES"]["SECTION_ID"];
	$cache_path = '/arIBlockSeoCatalogMenu_';
	if ($cache_time > 0 && $cache->InitCache($cache_time, $cache_id, $cache_path))
	{
		$res = $cache->GetVars();
		if (is_array($res["arSeoSect"]) && (count($res["arSeoSect"]) > 0)){
			$arSeoSect = $res["arSeoSect"];
		}
		$id_main = $res["id_main"];
	}
	if ( !is_array($arSeoSect) ){
		$rsParent = CIBlockSection::GetByID($arResult["VARIABLES"]["SECTION_ID"]);
		$arParent = $rsParent->Fetch();
		if($arParent["DEPTH_LEVEL"]>1 && (intval($arParent["RIGHT_MARGIN"])-intval($arParent["LEFT_MARGIN"])==1)){
			$id_main = $arParent['IBLOCK_SECTION_ID'];
			$seo_sect = true;
		}else{
		...
		$rsSeo = CIBlockSection::GetList($arOrder, $arFilter, false, array(
		...
		{		$arSeo['NAME'] = mb_ucfirst($arSeo['NAME']);
				if($arResult["VARIABLES"]["SECTION_ID"]==$arSeo["ID"]){
					$arSeo["CUR"] = true;
				}
				$arSeoSect["LIST"][] = $arSeo;
		}
		if ($cache_time > 0)
		{
			 $cache->StartDataCache($cache_time, $cache_id, $cache_path);
			 $cache->EndDataCache(array("arSeoSect"=>$arSeoSect, "id_main"=>$id_main));
		}
	}
	/*global $USER;
    if ($USER->IsAdmin()){*/
	/*if($arSeoSect['LIST']){?>
		...
		<?foreach($arSeoSect['LIST'] as $seoSection){?>
			<li><?if($seoSection["CUR"]){?><span><?}else{?><a href="<?=$seoSection["SECTION_PAGE_URL"]?>"><?}?><?=$seoSection["NAME"]?><?if($seoSection["CUR"]){?></span><?}else{?></a><?}?></li>
		<?}?>
		</ul>
	<?}*/
	//}
	...
	<div class="right_block clearfix catalog" id="right_block_ajax">
		...
		<div class="inner_wrapper">
			<?if($arSection["DEPTH_LEVEL"]>1):?>
				<?$APPLICATION->ShowViewContent('sotbit_seometa_linked_conditons');//вывод верхнего описания?>
			<?endif?>
			...
			<div class="sort_header view_<?=$display?>">
				<!--noindex-->
					<div class="sort_filter">
						...
						?>
						<a rel="nofollow" href="<?=$APPLICATION->GetCurPageParam('sort=clear', array('sort', 'order'))?>" class="sort_btn <?=($sort == "SHOWS"? 'current' : '')?>" rel="nofollow">
							<i class="icon" title="<?=GetMessage('SECT_SORT_DEFAULT')?>"></i><span><?=GetMessage('SECT_SORT_DEFAULT')?></span><i class="arr" style="opacity:0"></i>
						</a>
...
//"MIN_AMOUNT" => $arSection["UF_MIN_AMOUNT"],
//"MAX_AMOUNT" => $arSection["UF_MAX_AMOUNT"],
			...
			<?if($isAjax=="N"){?>
				<?$APPLICATION->IncludeComponent("sotbit:seo.meta", "inner_list_tags-block", Array(
						"FILTER_NAME" => $arParams["FILTER_NAME"],	// Имя фильтра
						"SECTION_ID" => $arSection["ID"],	// ID Секции
						"CACHE_TYPE" => "A",	// Тип кеширования
						"CACHE_TIME" => $arParams["CACHE_TIME"],	// Время кеширования (сек.)
						"COMPONENT_TEMPLATE" => ".default",
						"KOMBOX_FILTER" => "N",	// Используется фильтр kombox:filter
				...
				<?$sotbitSeoMetaBottomDesc = $APPLICATION->GetProperty("sotbit_seometa_bottom_desc");?>
				<?if(!empty($sotbitSeoMetaBottomDesc)){?>
					<div class="group_description_block bottom">
						<div><?echo $sotbitSeoMetaBottomDesc;?></div>
					</div>
				<?}else{?>
					<?if($arSection["UF_SECTION_SEO_DESCR"] && $sotbitSeoMetaBottomDesc==""):?>
						<div class="group_description_block bottom">
							<div><?=$arSection["UF_SECTION_SEO_DESCR"]?></div>
						</div>
					<?endif;?>
				<?}?>
				<div class="clear"></div>
...
<?}?>
<?$this->SetViewTarget("catalog_title_unt");?>
